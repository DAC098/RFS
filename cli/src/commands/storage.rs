...
pub fn create(state: &mut AppState, args: &ArgMatches) -> error::Result<()> {
    ...
    let type_ = match args.subcommand() {
        Some(("local", local_args)) => {
            let path = local_args.get_one::<PathBuf>("path")
                .map(|v| v.clone())
                .unwrap();

            lib::actions::storage::CreateStorageType::Local {
                path
            }
        },
        _ => unreachable!()
    ...
    let action = lib::actions::storage::CreateStorage {
        name, type_, tags
    };
    ...
    let result = res.json::<lib::json::Wrapper<lib::schema::storage::StorageItem>>()?;
...
pub fn update(state: &mut AppState, args: &ArgMatches) -> error::Result<()> {
    ...
    let type_action = match &current.type_ {
        lib::schema::storage::StorageType::Local(_) => {
            None
        }
    };
    ...
    let action = lib::actions::storage::UpdateStorage {
